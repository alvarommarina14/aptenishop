...
export default function UploadFile({
    ...
    const [images, setImages] = useState<ImageItem[]>(
        variantImages.map((img) => ({ type: 'server', data: img }))
    );
    const [selectedIndexes, setSelectedIndexes] = useState<number[]>([]);

    const updateFormFiles = (imageItems: ImageItem[]) => {
        const localFiles = imageItems
            .filter((img): img is LocalImage => img.type === 'local')
            .map((img) => img.file);
    ...
    const deleteSelected = async () => {
        ...
        const serverImages = toDelete.filter(
            (img): img is ServerImage => img.type === 'server'
        );
